//
// This file contains the UI functions to manage Tenant Syncronization
//
function ciniki_tenants_sync() {
    this.users = null;
    this.user = null;
    ...
    this.init = function() {
        ...
        this.syncs.sections = {
            'info':{'label':'Local System', 'list':{
                ...
                'uuid':{'label':'Tenant UUID', 'value':'unknown'},
            ...
            '_add':{'label':'', 'buttons':{
                'add':{'label':'Add Syncronization', 'fn':'M.ciniki_tenants_sync.showAdd();'},
                'numbers':{'label':'Check Table Numbers', 'fn':'M.ciniki_tenants_sync.showRowCounts(\'M.ciniki_tenants_sync.showInfo();\');'},
            }},
        };
        ...
        this.syncs.rowFn = function(s, i, d) { return 'M.ciniki_tenants_sync.showSync(\'M.ciniki_tenants_sync.showInfo();\',\'' + d.sync.id + '\');'; };
        this.syncs.noData = function() { return 'No syncronizations'; };
        this.syncs.addButton('add', 'Add', 'M.ciniki_tenants_sync.showAdd();');
        this.syncs.addClose('Back');

        //
        // Setup the panel to show the details of an sync
        //
        this.sync = new M.panel('Tenant Sync',
            'ciniki_tenants_sync', 'sync',
            'mc', 'medium', 'sectioned', 'ciniki.tenants.syncs.sync');
        this.sync.data = null;
        this.sync.sections = {
            'info':{'label':'', 'list':{
                'remote_name':{'label':'Remote', 'value':'unknown'},
                'remote_uuid':{'label':'Tenant UUID', 'value':'unknown'},
...
//              }},
        ...
        this.sync.addButton('edit', 'Edit', 'M.ciniki_tenants_sync.showEdit(\'M.ciniki_tenants_sync.showSync();\',M.ciniki_tenants_sync.sync.sync_id);');
        ...
        this.add = new M.panel('Add Sync',
            'ciniki_tenants_sync', 'add',
            'mc', 'medium', 'sectioned', 'ciniki.tenants.syncs.edit');
        this.add.data = null;
        this.add.sections = {
            '':{'label':'Remote Information', 'fields':{
                'remote_name':{'label':'System Name', 'value':'unknown', 'type':'text'},
                'remote_uuid':{'label':'Tenant UUID', 'value':'unknown', 'type':'text'},
            ...
            '_go':{'label':'', 'buttons':{
                'setup':{'label':'Initialize Sync', 'fn':'M.ciniki_tenants_sync.setupSync();'},
            }},
        };
        ...
        this.edit = new M.panel('Edit Sync',
            'ciniki_tenants_sync', 'edit',
            'mc', 'medium', 'sectioned', 'ciniki.tenants.syncs.edit');
        this.edit.data = null;
        this.edit.sections = {
            '':{'label':'Remote Information', 'fields':{
                'remote_name':{'label':'System Name', 'value':'unknown', 'type':'text'},
                'remote_uuid':{'label':'Tenant UUID', 'value':'unknown', 'type':'text', 'editable':'no'},
                'type':{'label':'Type', 'type':'multitoggle', 'toggles':{'push':'Push', 'pull':'Pull', 'bi':'Bi'}},
                'status':{'label':'Status', 'type':'multitoggle', 'toggles':{'10':'Active', '60':'Suspended'}},
            }},
            '_go':{'label':'', 'buttons':{
                'save':{'label':'Save', 'fn':'M.ciniki_tenants_sync.saveSync();'},
                'delete':{'label':'Delete', 'fn':'M.ciniki_tenants_sync.deleteSync();'},
            }},
        };
        this.edit.fieldValue = function(s, i, d) {
            return this.data[i];
        };
        this.edit.addButton('save', 'Save', 'M.ciniki_tenants_sync.saveSync();');
        this.edit.addClose('Back');

        //
        // The numbers panel shows tables from each sync, and the item counts
        // for the tenant.  This helps with debugging problems.
        //
        this.rowcounts = new M.panel('Table Numbers',
            'ciniki_tenants_sync', 'rowcounts',
            'mc', 'medium', 'sectioned', 'ciniki.tenants.syncs.rowcounts');
    ...
    this.showInfo = function(cb) {
        ...
        //
        var rsp = M.api.getJSONCb('ciniki.tenants.syncInfo', 
    ...
    this.showSync = function(cb, id) {
        ...
        // Get the sync information for this server and tenant
        //
        var rsp = M.api.getJSONCb('ciniki.tenants.syncDetails', 
            {'tnid':M.curTenantID, 'sync_id':this.sync.sync_id}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
                var p = M.ciniki_tenants_sync.sync;
                p.data = rsp.sync;

                // Show or hide sync button depending on type of sync
                p.sections._buttons.buttons.ping.fn = 'M.ciniki_tenants_sync.syncPing(\'' + p.sync_id + '\');';
                if( rsp.sync.status == 10 ) {
                    p.sections._buttons.buttons.sync_incremental.visible = 'yes';
                    p.sections._buttons.buttons.sync_incremental.fn = 'M.ciniki_tenants_sync.syncNow(\'' + p.sync_id + '\',\'incremental\');';
                    p.sections._buttons.buttons.sync_partial.visible = 'yes';
                    p.sections._buttons.buttons.sync_partial.fn = 'M.ciniki_tenants_sync.syncNow(\'' + p.sync_id + '\',\'partial\');';
                    p.sections._buttons.buttons.sync_full.visible = 'yes';
                    p.sections._buttons.buttons.sync_full.fn = 'M.ciniki_tenants_sync.syncNow(\'' + p.sync_id + '\',\'full\');';
                    p.sections._buttons.buttons.check.visible = 'yes';
                    p.sections._buttons.buttons.check.fn = 'M.ciniki_tenants_sync.syncCheck(\'' + p.sync_id + '\');';
                } else {
    ...
    this.showAdd = function() {
        this.add.cb = 'M.ciniki_tenants_sync.showInfo();';
        this.add.show();
    }

    this.setupSync = function() {
        var c = this.add.serializeForm('yes');
        var rsp = M.api.postJSONCb('ciniki.tenants.syncSetupLocal', 
            {'tnid':M.curTenantID}, c, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
                M.ciniki_tenants_sync.add.close();
            });
    }
    ...
    this.syncPing = function(id) {
        ...
        // Get the sync information for this server and tenant
        //
        var rsp = M.api.getJSONCb('ciniki.tenants.syncPing', 
            {'tnid':M.curTenantID, 'sync_id':id}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
    ...
    this.syncCheck = function(id) {
        ...
        // Get the sync information for this server and tenant
        //
        var rsp = M.api.getJSONCb('ciniki.tenants.syncCheck', 
            {'tnid':M.curTenantID, 'sync_id':id}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
    ...
    this.syncNow = function(id, type) {
        ...
        // Get the sync information for this server and tenant
        //
        var rsp = M.api.getJSONCb('ciniki.tenants.syncNow', 
            {'tnid':M.curTenantID, 'sync_id':id, 'type':type}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
                M.ciniki_tenants_sync.showSync(null, id);
            });
    };
    ...
    this.showEdit = function(cb, id) {
        ...
        var rsp = M.api.getJSONCb('ciniki.tenants.syncDetails', 
            {'tnid':M.curTenantID, 'sync_id':this.edit.sync_id}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                }
                var p = M.ciniki_tenants_sync.edit;
                p.data = rsp.sync;
                p.refresh();
                p.show(cb);
            });
    ...
    this.saveSync = function() {
        ...
        if( c != '' ) {
            var rsp = M.api.postJSONCb('ciniki.tenants.syncUpdate', 
                {'tnid':M.curTenantID, 'sync_id':M.ciniki_tenants_sync.sync.sync_id}, c, function(rsp) {
                    if( rsp.stat != 'ok' ) {
                        M.api.err(rsp);
                        return false;
                    } 
                    M.ciniki_tenants_sync.edit.close();
                });
        } else {
            this.edit.close();
        }
    ...
    this.showRowCounts = function(cb) {
        var rsp = M.api.getJSONCb('ciniki.tenants.syncCheckRowCounts', 
            {'tnid':M.curTenantID}, function(rsp) {
                if( rsp.stat != 'ok' ) {
                    M.api.err(rsp);
                    return false;
                } 
                var p = M.ciniki_tenants_sync.rowcounts;
